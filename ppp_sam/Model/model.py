...
def build_P3SAM(self, model_file_path: str):  # build p3sam
    ######################## Sonata ########################
    self.sonata = sonata.load(model_file_path)
...
def load_state_dict(
    ...
    if ckpt_path is not None:
        ...
        ckpt_path = hf_hub_download(
            repo_id="tencent/Hunyuan3D-Part", filename="p3sam.ckpt", local_dir="weights"
        )
    ...
    if ignore_seg_mlp and seg_mlp_flag:  # ignore seg_mlp
        print(
            "seg_mlp is missing in loaded state dict, ignore seg_mlp in loaded state dict"
        )
    if ignore_seg_s2_mlp and seg_s2_mlp_flag:  # ignore seg_s2_mlp
        print(
            "seg_s2_mlp is missing in loaded state dict, ignore seg_s2_mlp in loaded state dict"
        )
    if ignore_iou_mlp and iou_mlp_flag:  # ignore iou_mlp
        print(
            "iou_mlp is missing in loaded state dict, ignore iou_mlp in loaded state dict"
        )
